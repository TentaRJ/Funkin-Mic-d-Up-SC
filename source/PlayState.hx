	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (SONG.song.toLowerCase())
			{
				...
				case 'tutorial':
					babyArrow.frames = Paths.getSparrowAtlas('FIRE', 'shared');
...
private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && !boyfriend.stunned && generatedMusic && !frozen && !ended)
		{
		...
		if ((up || right || down || left) && !boyfriend.stunned && generatedMusic && !frozen && !ended || (upP || downP || leftP || rightP) && !boyfriend.stunned && generatedMusic && !frozen && !ended)
		{
			notes.forEachAlive(function(daNote:Note)
			{
